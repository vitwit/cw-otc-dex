...
export  const extractDenomsAndDecimals = () => {
    const result = [];
     
    networks.forEach(network => {
      // Extract denominations from currencies
      network.config.currencies.forEach(currency => {
        result.push({
      ...
      // Extract denominations from feeCurrencies
      network.config.feeCurrencies.forEach(feeCurrency => {
        result.push({
      ...
      // Extract denomination from stakeCurrency
      if (network.config.stakeCurrency) {
        result.push({
          coinDenom: network.config.stakeCurrency.coinDenom,
          coinMinimalDenom: network.config.stakeCurrency.coinMinimalDenom,
          coinDecimals: network.config.stakeCurrency.coinDecimals,
        });
      }
    });
